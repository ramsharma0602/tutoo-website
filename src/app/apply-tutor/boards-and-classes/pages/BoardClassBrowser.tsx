...
const BOARD_COLORS: Record<string, { color: string; bg: string; border: string; grad: string }> = {
  CBSE:  { color: "#7B2FF7", bg: "rgba(123,47,247,0.09)",  border: "rgba(123,47,247,0.2)",  grad: "from-[#7B2FF7] to-[#7B2FF7]" },
  ICSE:  { color: "#7B2FF7", bg: "rgba(248,120,8,0.09)", border: "rgba(248,120,8,0.2)", grad: "from-[#EA580C] to-[#C2410C]" },
    ...
    <div className="bg-[#FAFAFC] min-h-screen">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border mb-7"
              style={{ background: "rgba(248,120,8,0.08)", borderColor: "rgba(248,120,8,0.25)", backdropFilter: "blur(10px)" }}>
              <BookOpen className="w-4 h-4 text-[#7B2FF7]" />
              <span className="text-sm font-bold text-[#059669]">Board & Classes</span>
            </div>
            <h1 className="text-5xl lg:text-6xl font-black leading-[1.1] tracking-tight text-[#0A1028] mb-5"
            ...
            <p className="text-xl text-[#6E6A85] max-w-2xl mx-auto">
          ...
          {BOARDS.map((b) => {
            ...
            return (
              <motion.button key={b.board}
                ...
                style={isActive
                  ? { background: `linear-gradient(135deg, ${c.color}, #7B2FF7)`, color: "#fff", border: "1px solid transparent", boxShadow: `0 4px 20px ${c.color}40` }
                  : { background: "rgba(255,255,255,0.85)", color: "#1E1B3A", borderColor: "rgba(30,27,58,0.09)", backdropFilter: "blur(8px)" }}
              >
                {b.board} Board
              </motion.button>
            );
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 pb-24">
              ...
              return (
                <div key={cat} className="mb-12">
                  <div className="flex items-center gap-3 mb-5">
                    <span className="text-base font-bold text-[#1E1B3A]" style={{ fontFamily: "var(--font-heading)" }}>
                      {CAT_LABEL[cat]}
                    </span>
                    <div className="flex-1 h-px bg-gradient-to-r from-[rgba(30,27,58,0.1)] to-transparent" />
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {classes.map((cls, i) => (
                      ...
                      onClick={() => navigate(slugToPath(cls.slug))}
                        ...
                        style={{ background: "rgba(255,255,255,0.88)", borderColor: "rgba(30,27,58,0.07)", backdropFilter: "blur(10px)", boxShadow: "0 2px 10px rgba(30,27,58,0.04)" }}
                      >
                        ...
                        <p className="text-sm font-bold text-[#1E1B3A] mb-1 group-hover:text-[#7B2FF7] transition-colors duration-250"
                          style={{ fontFamily: "var(--font-heading)" }}>
                          {cls.label}
                        </p>
                        <p className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider mb-3">{selectedBoard}</p>
                        <div className="flex items-center gap-1 text-xs font-bold text-[#7B2FF7] group-hover:text-[#7B2FF7] transition-colors duration-250">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          style={{ background: "linear-gradient(135deg, #0A1028, #111827)" }}>
          <div className="absolute inset-0 rounded-3xl" style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(248,120,8,0.15), transparent 60%)" }} />
          <div className="absolute top-0 left-0 right-0 h-[2px] rounded-t-3xl bg-gradient-to-r from-[#7B2FF7] via-[#7B2FF7] to-[#7B2FF7]" />
          <div className="relative z-10">
            ...
            <button className="group h-12 px-8 rounded-2xl bg-gradient-to-r from-[#EA580C] to-[#C2410C] text-white font-bold text-sm shadow-lg hover:scale-[1.02] transition-all duration-300 flex items-center gap-2 mx-auto">
